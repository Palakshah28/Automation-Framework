...
namespace ContradoRegressionSuite.Global
{
    class GlobalDefinitions
    {
        ...
        //Generic Methods
        //Load URL
        public static void loadURL(IWebDriver driver, string SiteCulture)
        {
            try
            {
                driver.Navigate().GoToUrl(SiteCulture + ".rockycloud.co.uk");
                Global.Base.test.Log(RelevantCodes.ExtentReports.LogStatus.Pass, "Successfully Launched URL of " + SiteCulture);

            }
            catch (Exception e)
            {
                Global.Base.test.Log(RelevantCodes.ExtentReports.LogStatus.Fail, "Issue in launching URL of  " + SiteCulture + e.Message);
            }
        ...
        public static void TextBox(IWebDriver driver, string Locator, string LocatorValue, string InputValue)
        {
            try
            {
                ...
                Global.Base.test.Log(RelevantCodes.ExtentReports.LogStatus.Pass, "Entered data: " + InputValue);

            }
            catch (Exception e)
            {
                Global.Base.test.Log(RelevantCodes.ExtentReports.LogStatus.Fail, "Not able to enter data: " + InputValue + " " + e.Message);
            }
        ...
        public static void ActionButton(IWebDriver driver, string Locator, string LocatorValue)
        {
            try
            {
                ...
                {
                    wait(10);
                    if (ElementVisible(driver, Locator, LocatorValue))
                    {
                        driver.FindElement(By.XPath(LocatorValue)).Click();
                    }
                ...
                // Add log in Extent Report
                Global.Base.test.Log(RelevantCodes.ExtentReports.LogStatus.Pass, "Clicked on button: " + LocatorValue);
            }
            catch(Exception e)
            {
                Global.Base.test.Log(RelevantCodes.ExtentReports.LogStatus.Fail, "Not able to click on button: " + LocatorValue);
            }
        }
